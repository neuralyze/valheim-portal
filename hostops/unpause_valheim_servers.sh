...
echo "Unpausing all Valheim Servers"
...
for world in "${WORLD_NAMES[@]}"; do
	[[ -n $world ]] || continue
	"$SCRIPT_DIR/unpause_valheim_server.sh" "$world" &
	pids+=($!)
	worlds+=("$world")
done
...
echo "${#pids[@]} world(s) unpausing. Waiting for them to finish..."
...
for index in "${!pids[@]}"; do
	...
	if ((status != 0)); then
		echo "unpause of ${worlds[index]} failed with status $status" >&2
		failures=$((failures + 1))
	fi
done

((failures == 0)) || { echo "$failures of ${#pids[@]} world(s) failed" >&2; exit 1; }
echo "Unpaused ${#pids[@]} world(s)."

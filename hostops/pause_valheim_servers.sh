...
echo "Pausing all Valheim Servers"
...
for world in "${WORLD_NAMES[@]}"; do
	[[ -n $world ]] || continue
	"$SCRIPT_DIR/pause_valheim_server.sh" "$world" &
	pids+=($!)
	worlds+=("$world")
done
...
echo "${#pids[@]} world(s) pausing. Waiting for them to finish..."
...
for index in "${!pids[@]}"; do
	...
	if ((status != 0)); then
		echo "pause of ${worlds[index]} failed with status $status" >&2
		failures=$((failures + 1))
	fi
done

((failures == 0)) || { echo "$failures of ${#pids[@]} world(s) failed" >&2; exit 1; }
echo "Paused ${#pids[@]} world(s)."

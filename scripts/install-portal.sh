...
# Spool the portal stages an uploaded world save in, shared with the agent. It is not
# under the world root: the portal mounts that read-only, and a staged save is not a
# world until provisioning has placed it.
PORTAL_WORLD_UPLOAD_DIR=${PORTAL_WORLD_UPLOAD_DIR:-/var/lib/valheim-world-uploads}
...
Optional configuration worth knowing about:
  ...
  PORTAL_VHVR_SOURCE_URL      Source-code link for ValheimVR, on the pages that
                              hand out ValheimVRMod.dll. GPL-3.0, a separate
                              program from the one above. Defaults to our fork;
                              set it if you build that DLL from your own tree.
...
# The wake file lives in the portal's own data volume, which is the only writable
# path it has: every other mount is read-only by design. Set only when there is a
# runner to wake AND the poller is off - with both, one operator message would start
# a pass in each and two passes could answer the same turn.
container_agent_wake_path() {
  [[ ${PORTAL_ENABLE_AGENT_BRIDGE,,} == true && ${AGENT_RUNNER_SERVICE,,} != true ]] &&
    printf '/var/lib/valheim-portal/agent-wake'
}
...
PORTAL_WORLD_UPLOAD_DIR=$PORTAL_WORLD_UPLOAD_DIR
...
# The world upload spool is the only directory the portal container writes host-visible
# bytes into, and the agent both reads and removes what lands there. It is owned by the
# agent account and group; the portal container joins that group through group_add in
# compose.yaml, so setgid keeps a staging directory created by the portal readable and
# removable by the agent afterwards. Nobody else has any business in it: a staged save
# is a complete copy of somebody's world.
ensure_world_upload_spool() {
  # staged() so a --install-root dry run or the staged test in scripts/tests never
  # creates or chowns a directory outside its own prefix. The generated unit and env
  # files still name the real path, which is what the running deployment uses.
  local dir
  dir=$(staged "$PORTAL_WORLD_UPLOAD_DIR")
  run mkdir -p -- "$dir"
  if [[ -z $install_root ]]; then
    run chown "$AGENT_USER:$AGENT_GROUP" "$dir"
    run chmod 2770 "$dir"
  fi
  note "world upload spool ready at $dir"
}
...
# "Hit send and it answers", without giving the portal any host access.
#
# The portal writes a file in its data volume; systemd watches the host side of that
# same file and starts one runner pass. Installed after the portal, because the host
# path only exists once docker has created the volume. Absent volume or absent bridge
# means no unit and manual triggering, which is a working deployment, not a failure.
write_wake_unit() {
  [[ -n $(container_agent_wake_path) ]] || return 0
  step "Agent runner wake"
  if [[ -n $install_root ]]; then
    note "staging prefix set; not resolving the data volume"
    return 0
  fi
  local mountpoint host_wake
  mountpoint=$(docker volume inspect valheim-portal_portal-data --format '{{.Mountpoint}}' 2>/dev/null || true)
  if [[ -z $mountpoint ]]; then
    warn "cannot resolve the portal data volume; runner passes stay manual. Re-run this installer once the portal has started."
    return 0
  fi
  host_wake=$mountpoint/agent-wake
  install_file "$(resolved_unit)/valheim-agent-runner-wake.path" 0644 root:root <<UNIT
# Generated by scripts/install-portal.sh. Do not edit; re-run the installer.
#
# Starts one runner pass when the operator sends a message or decides a verb. The
# portal writes $(container_agent_wake_path) inside its container; this is the same
# file seen from the host.
[Unit]
Description=Wake the Valheim Portal agent runner when the operator acts

[Path]
PathModified=$host_wake
Unit=valheim-agent-runner-once.service

[Install]
WantedBy=multi-user.target
UNIT
  run systemctl daemon-reload
  run systemctl enable --now valheim-agent-runner-wake.path
  note "watching $host_wake; sending a message now starts a pass"
}
...
start_runner() {
  ...
  run systemctl daemon-reload
  if [[ ${AGENT_RUNNER_SERVICE,,} == true ]]; then
    # Two triggers for one job would let a message be answered twice.
    run systemctl disable --now valheim-agent-runner-wake.path || true
...
# Read by hostops/lib/common.sh when provisioning is handed a world upload staging
# id. The portal writes the staged save pair here; the agent copies it into the new
# world and removes it.
AGENT_WORLD_UPLOAD_ROOT=$PORTAL_WORLD_UPLOAD_DIR
...
ProtectHome=true
ProtectSystem=strict
ReadWritePaths=$PORTAL_AGENT_SOCKET_DIR $VALHEIM_WORLD_ROOT $PORTAL_WORLD_UPLOAD_DIR
...
uninstall() {
  ...
  run rm -f -- "$(resolved_unit)/valheim-portal-agent.service" \
    ...
    "$(resolved_unit)/valheim-agent-runner-wake.path"
  if [[ -z $install_root ]]; then
    run systemctl daemon-reload
  fi

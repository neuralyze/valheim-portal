...
def locate(row: dict) -> Path | None:
    ...
    if row["body"] == "committed":
        candidate = BODIES / row["origin"].replace("/", "__") / row["name"]
        return candidate if candidate.is_file() else None
    if row["origin"] == "fleet corpus":
        corpus = json.loads((DATA / "corpus_roles.json").read_text("utf-8"))
        for entry in corpus["entries"]:
            if entry["file"] == row["name"]:
                path = Path(entry["source"])
                return path if path.is_file() else None
        return None
    candidate = STAGING / row["origin"].replace("/", "__") / row["name"]
    return candidate if candidate.is_file() else None
...
def main() -> int:
    ...
    ap.add_argument("--name", nargs="*", default=[], help="exact file names")
    ...
    if args.verify:
        present = missing = drifted = empty = 0
        for row in rows:
            path = locate(row)
            if path is None:
                missing += 1
                print(f"MISSING  {row['name']}  ({row['origin']})")
                continue
            if not row["sha256"]:
                empty += 1
                continue
        ...
        print(f"{present} verified, {drifted} drifted, {missing} unavailable, {empty} zero-byte")
    ...
    for row in rows:
        ...
        if path is None:
            ...
            skipped += 1
            continue
        if row["sha256"] and sha256(path.read_bytes()) != row["sha256"]:

...
def render_template(world: World, preset, part: Partition, spawn: list[list[int]]) -> str:
    ...
    lines += [
        ...
        "#                 DELIBERATELY OMITTED below, not forgotten: Lumberjacking",
        "#                 hard-patches GetSkill(WoodCutting) to a level-0 dummy and",
        "#                 IsSkillValid(WoodCutting) to false, so it is permanently 0 and",
        "#                 ImpactfulSkills' WoodCuttingLootFactor = 3 is dead code.",
        "#                 Do not add it back.",
        "#   berries etc   Smoothbrain `Foraging` AND ImpactfulSkills' own `Farming`.",
        "#   crops         ImpactfulSkills' `Farming` only -- Smoothbrain Foraging's scope",
        ...
        "# AMBIGUITY, unresolved: `Farming` is both a vanilla 1.0 skill (id 15) and",
        "# ImpactfulSkills' custom skill (SkillType 106). Only the custom one moves yield,",
        "# and which one CheatRaiseSkill(\"Farming\", v) resolves to on this load order has",
        "# NOT been verified. Confirm before trusting the crop half of a tier's economy.",
        "",
    ]

...
- The bridge token is a bearer for the whole agent surface for as long as it is valid. It grants no mutation by itself - every mutating verb still waits for an operator - but it does grant reads, which include world status and mod inventories. Rotate `/etc/valheim-portal/agent-bridge-token` after suspected exposure and restart the portal and the runner.

...
| `tools/valheim_provision.py` | 15 positionals: `WORLD SERVER_NAME PORT PUBLIC CROSSPLAY PLAYER_LIMIT PRESET BACKUP_INTERVAL BACKUP_AGE BACKUP_COUNT PROFILE SEED SOURCE_WORLD TEMPLATE_WORLD TEMPLATE_PROFILE` | - | new world directory tree, `valheim.env`, port allocations | Transactionally creates a portal-managed world: directory layout, `valheim.env`, mod profile seeding from a template, and host port allocation (two UDP game ports plus status, supervisor and discord TCP ports). Reads the server password from `PORTAL_SERVER_PASSWORD`. |

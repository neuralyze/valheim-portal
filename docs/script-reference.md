...
| `hostops/backup_valheim_world.sh` | `WORLD [BACKUP_NAME]` | backup archives | `backup` | Tars the live world save into `$VALHEIM_ROOT/world_backups/world-<WORLD>-<name>-<timestamp>.tgz`, detecting the save format. A Valheim 1.0 world is the directory `worlds_local/<WORLD>/` holding a `*.fwl2`, and the archive is that one directory member plus its contents at their original basenames; a 0.220.x world is the `<WORLD>.db`/`<WORLD>.fwl` pair, and the archive is still exactly those two members in that order, in whichever of the two casings the pair uses. Detection is positive on both sides, so the game's own `<WORLD>_backup_auto-*` copies - directories under 1.0, files under 0.220.x - are never mistaken for the live save and are never archived. A world with neither layout is refused by name instead of failing inside `tar`. `BACKUP_NAME` defaults to `backup` and must match `[A-Za-z0-9][A-Za-z0-9._-]*`, because `restore_valheim_world.sh` parses it back out of the filename. Prints the archive path. Requires `VALHEIM_ROOT`. |
...
| `hostops/portal_admin_mode.sh` | `WORLD PROFILE {on|off|state}` | the world's `mods/admin-mode/` plugin overlay | `admin_mode_on`, `admin_mode_off` | Arms, disarms or reports one world's admin-mode plugin overlay: `JereKuusela-Structure_Tweaks` and `Azumatt-PerfectPlacement`, which disconnect every connected player when they are loaded server-side. Staging only - it never stops, deploys or starts anything, because the ordering that makes a maintenance window safe is composed by the agent from the scripts that already do each step. `off` refuses nothing, since it is the path that gets players back in. `state` reports what is staged on disk and is a hand-run action with no agent operation behind it: the portal's own record is the operator's decision, and the two are compared by hand during a recovery. Execs `tools/valheim_mods.py admin-mode`. |
...
| `hostops/provision_valheim_server.sh` | `WORLD SERVER_NAME PORT PUBLIC CROSSPLAY PLAYER_LIMIT PRESET BACKUP_INTERVAL BACKUP_AGE BACKUP_COUNT PROFILE SEED SOURCE_WORLD COPY_FROM WORLD_UPLOAD` (exactly 15) | new world directory tree | `provision` | Transactionally creates a portal-managed world directory via `tools/valheim_provision.py`. Requires the `PORTAL_SERVER_PASSWORD` environment variable; refuses to run without it. `SEED`, `SOURCE_WORLD` and `WORLD_UPLOAD` are the three world sources and at most one may be set. `WORLD_UPLOAD` is a 32-character hex staging id, never a path: this script resolves it against the world upload root (`VALHEIM_WORLD_UPLOAD_ROOT`, or `AGENT_WORLD_UPLOAD_ROOT` as the installer writes it) and hands `valheim_provision.py` the resolved directory, which holds the validated `world.db`/`world.fwl` pair the portal staged. A wrong argument count prints the full positional order rather than a bare count; `python3 tools/valheim_provision.py --help` explains each one. `hostops/tests/agent_argv_contract.sh` holds this list against the argv `internal/agent/agent.go` actually sends. |
...
| `hostops/restore_valheim_world.sh` | `WORLD BACKUP_NAME` | world save | `restore` | Restores a world save from a named archive in the backup inventory. The archive's own members decide the format, because the live directory may be empty or on the other format: a two-member `.db`/`.fwl` pair is installed file by file mode 0640, and a 1.0 directory archive replaces `worlds_local/<WORLD>/` wholesale, taking ownership and 0775/0664 modes from the live `worlds_local` so the container can still write the next save generation. Either way the stem must be the world's own casing or its lowercase form, so an archive holding another world's save is refused. Staging happens in a private directory under `config_merged`, never inside `worlds_local`: on 2026-09-12 a directory that was not a valid world, parked in `worlds_local`, left the 1.0 server hanging in the start scene forever. Requires `VALHEIM_ROOT`. |
...
| `tools/valheim_provision.py` | 14 positionals: `WORLD SERVER_NAME PORT PUBLIC CROSSPLAY PLAYER_LIMIT PRESET BACKUP_INTERVAL BACKUP_AGE BACKUP_COUNT PROFILE SEED SOURCE_WORLD COPY_FROM` | - | new world directory tree, `valheim.env`, port allocations | Transactionally creates a portal-managed world: directory layout, `valheim.env`, linking the server to the profile it runs (created empty, or copied from `COPY_FROM`, when it does not exist), and host port allocation (two UDP game ports plus status, supervisor and discord TCP ports). Reads the server password from `PORTAL_SERVER_PASSWORD`. `SOURCE_WORLD` copies whichever save format the source world uses: a 0.220.x `.db`/`.fwl` pair, or a Valheim 1.0 `worlds_local/<WORLD>/` directory, whose members are copied under their original basenames with the new world's name rewritten into every `_main.<N>.fwl2`. `WORLD_UPLOAD` still stages a `.db`/`.fwl` pair only, because `internal/app/world_upload.go` requires the uploaded archive to hold one. |
| `tools/valheim_world.py` | see subcommands | `inspect PATH`, `generate PATH NAME SEED [--templates DIR] [--force]`, `clone SOURCE DESTINATION NAME [--force]` | `.fwl` metadata files (`generate`, `clone` only) | Reads and writes Valheim `.fwl` world metadata: seed, world version, generator version. Valheim 1.0's `_main.<N>.fwl2` is the same container with world version 41, so `inspect` and `clone` read and rewrite one unchanged. `inspect` is read-only and emits JSON. `generate` and `clone` refuse an existing destination unless `--force`, and print the path and seed they wrote. `--templates` scans both layouts for the newest world and generator version. |

...
| `profile-definition-builder` | `-source-manifest -world -profile -client-type -audience -config-dir -output` (`-flat-companion`, `-package-base-url`, `-true-nonvr`, `-debug-logging`, `-config-authority`, `-config-authority-database`, `-version`) | Writes the profile-definition ZIP at `-output`; prints that path on stdout | Turns a shared `profile-manifest.json` plus a client-config directory into one checksum-bound profile definition. `-client-type` is `flat` or `vr`. `-audience` is `player` or `admin` and has no default; it is **validated here but never written into the archive** — the audience is a portal-only fact and reaches `releases.audience` through `seed-release` instead (see [release-format.md](release-format.md#the-definition-format-is-frozen)). Config backups (`.before-`, `.bak`, `_changed.`, trailing `~`) are skipped rather than shipped to players. Flat profiles require `-flat-companion` unless `-true-nonvr` strips the ValheimVR packages instead. VR profiles reject `-flat-companion`. `-package-base-url` defaults to `VALHEIM_PACKAGE_BASE_URL`. `-output` may not sit inside `-config-dir`. Either `-config-authority` (a `world-config-authority/v1` file) or `-config-authority-database` (the portal SQLite, read through `ExportWorldConfigAuthority`) layers the world's managed settings over `-config-dir` per KEY: a key with a record is rewritten in place, a key without one is left byte-identical, and the build emits `config/settings-baseline.json` recording exactly what it wrote. The two are mutually exclusive; neither means no layering and no baseline, which is the artifact this builder produced before the feature. `-version` is recorded in that manifest only, so the publisher and the baseline agree about which release seeded a client (see [release-format.md](release-format.md#why-the-settings-baseline-lives-under-config)). |

...
A **mod profile** is one shared definition stored once at `<fleet>/profiles/<name>`, owned by no world. A server links to one by name in `<world>/mods/.active-mod-profile`, so several servers can run the same definition and editing it reaches every one of them at its next restart. Three primaries exist: `flat` is the base set, `vr` is the base plus the VR compatibility shims and minus the mod whose UI needs a keyboard, and `admin` is the base plus the admin tools. Servers link to `admin`, because a server must run the superset of what any client may load.

A **published edition** is a different object, built from a profile rather than being one. Four are published per server: a headset edition from `vr`, two Flat editions from `flat` differing only in whether the ValheimVR companion is attached, and an admin Flat edition from `admin`. `ValheimVR-ValheimVR` is excluded from every profile manifest, so VR capability reaches a client only as a separately built, checksum-validated artifact — which is why a release target declares `valheim_vr` explicitly instead of inferring it from a name.

A published profile release is selected by world, profile, and client type. Steam world membership gates browser pages, device authorization, manifests, and profile definitions. The release row also records an `audience`, `player` or `admin` — in the `releases.audience` column, never in the published definition, which an installed client would refuse to parse — and an `admin` edition is offered only to a login that `isAdmin` accepts, so a player who can see the world is not shown a card that installs a developer console. See [release-format.md](release-format.md) for why the definition format is frozen.
...
Read-only profile-catalog and FWL-metadata operations return size-limited valid JSON. The creation wizard uses the catalog to name the profile a new server **runs**: the field is labelled "Profile this server runs", it defaults to the profile the most servers already run, and each choice is annotated with how many servers run it. Naming a profile that exists links this server to it; naming a new one creates it, empty or copied from a profile selected separately. Player pages derive the displayed seed from current FWL metadata rather than copied database state.
